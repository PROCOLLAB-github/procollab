...
Production backend contract: `907ced23273e298314e9d8604dbaabb25ded6869`.
...
метрик: «Назначений», «Выполнено», «Ожидает». Нулевые метрики остаются видимыми,
но неактивными. Доступ окончательно проверяет backend. Новых маршрутов нет.

### API и архитектура

- `GET /programs/:programId/project-analytics/` — сводка, включая `attention.delayedExperts`.
...
Ручной smoke перед production release: все три scope, detail/back,
...
Контракт доступен в production backend `907ced23273e298314e9d8604dbaabb25ded6869`.
...
- `getManagerParticipantsWithoutTeam` / `GetProgramManagerParticipantsWithoutTeamUseCase`:
  `GET /programs/:programId/project-analytics/participants-without-team/`.
- `getManagerProjectsAwaitingEvaluation` / `GetProgramManagerProjectsAwaitingEvaluationUseCase`:
  `GET /programs/:programId/project-analytics/projects-awaiting-evaluation/`.
...
Ручной production smoke и mobile/keyboard проверяются отдельно от unit/component tests.
...
Её стадии и значения не меняются: «Создано», «Не сдано», «Сдано»,
...
→ HTTP adapter → `GET /programs/:programId/project-analytics/projects-not-submitted/`.
...
## Кейсы в project analytics

Карточка «Кейсы» напрямую использует обязательный `ProgramAnalyticsOverview.cases`
из того же `GET /programs/:programId/project-analytics/`, без дополнительного запроса.
Production backend contract: `907ced23273e298314e9d8604dbaabb25ded6869`.
