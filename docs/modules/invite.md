...
```ts
interface SendForUserCommand { userId; projectId; role: string; specialization?: string; }
interface UpdateInviteCommand { inviteId; role: string; specialization?: string; }
```
