...
В конструкторе подписывается на собственный поток и **логирует каждое событие** через `LoggerService.debug("[EventBus] <type>", event.payload)`.

Используется чтобы один модуль уведомил другой о событии без прямой зависимости. Например, репозиторий проекта слушает `TaskAnswerSubmitted` от модуля курсов и инвалидирует свой кеш.
...
export const taskAnswerSubmitted = (taskId: number) => ({
