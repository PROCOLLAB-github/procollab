...
Актуальный результат доработки: [композиция, новости, окончательные проверки и локальная интеграция](followup/README.md).
...
- Участник: кейс, название и переход к проекту; текущая «Проверка» без галочки, конечное серверное состояние «Оценён» завершает все три этапа. Черновик — «Не отправлен». Без проекта доступна существующая явная команда создания заявки, если срок подачи открыт. До нажатия бизнес-действия не выполняются.
...
CSS фиксирует высоту 140,9844 px; фактическое значение Chromium — 140,984375 px. Подписи метрик 10 px. Многоточием сокращаются только динамические названия; MatTooltip доступен при hover и клавиатурном фокусе. Срок эксперта занимает две строки: подпись и полное значение с единицей времени. Служебные статусы видны целиком. Внутренней прокрутки нет.

В окончательной доработке проверены [23 состояния](followup/geometry.json), все три роли и их нулевые состояния, полный срок без наведения, hover/focus, Retry и адаптивные границы. [Актуальные скриншоты и кропы без масштабирования](followup/README.md#скриншоты). Измерения: [исходные](baseline-geometry.json), [окончательные](followup/geometry.json). Скриншоты ниже сохранены как история первоначальной реализации.

Fixture smoke использует TestBed и не вызывает API. Отдельно выполнена [реальная локальная интеграционная проверка](followup/README.md#локальная-интеграция): вход, новости, кейс, сдача, оценка 0, переходы и ошибки на отдельной PostgreSQL БД. Живой DEV и PROD не использовались.
...
Открыть `http://localhost:4302/?view=participant` при указанном viewport. Полный перечень `view` приведён в [отчёте доработки](followup/README.md#скриншоты). Fixtures находятся вне production entry point и не входят в обычную сборку приложения.

## Первоначальные проверки 14.09.2026 (история)

Ниже результаты до доработки композиции. [Окончательные проверки и сравнение полных suite на base/head](followup/README.md#окончательные-проверки-angular) заменяют их при приёмке.
...
Окончательные [адаптивные границы](followup/responsive.json), [Retry](followup/retry.json), [hover кейса](followup/screenshots/long-hover.png) и [клавиатурный focus проекта](followup/screenshots/long-focus.png) проверены повторно после доработки.
