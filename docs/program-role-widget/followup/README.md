...
Ограничения: живой DEV не проверялся. Последующая [диагностика `PUT /projects/NaN/ → 404`](../nan-diagnostic/README.md) установила чтение ID из root ActivatedRoute и воспроизвела тот же дефект на точном base и head; новый вход виджета его не усиливает. Обычное название и case сохраняются явной кнопкой, но удаление презентации оставляет старый URL даже после сообщения об успехе: отдельная [задача P1 редактора](../nan-diagnostic/editor-task.md), препятствие для его дальнейшего релиза. Код редактора в этом follow-up не исправлялся. Многопрограммный проект, все пограничные комбинации доступа и поздние ответы проверены автоматическими тестами; браузерный интеграционный сценарий не повторяет каждую комбинацию. Обновление EventBus без ухода со страницы проверено unit-тестами; в браузере подтверждён результат после возврата из существующих редакторов.
