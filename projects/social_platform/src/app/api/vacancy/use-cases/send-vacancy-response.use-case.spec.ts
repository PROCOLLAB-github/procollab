...
describe("SendVacancyResponseUseCase", () => {
  ...
  const payload = { whyMe: "test", accompanyingFile: "https://example.test/cv.pdf" };
  ...
  it("делегирует vacancyId и body, получает вакансию и эмитит событие", () =>
    new Promise<void>(done => {
      ...
      useCase.execute(10, payload).subscribe(() => {
        expect(repo.sendResponse).toHaveBeenCalledExactlyOnceWith(10, payload);
  ...
  it("при успехе возвращает ok<void>", () =>
    new Promise<void>(done => {
      ...
      useCase.execute(10, payload).subscribe(result => {
