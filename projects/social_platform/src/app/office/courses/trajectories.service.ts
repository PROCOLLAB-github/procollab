...
export class TrajectoriesService {
  ...
  getTrajectories(limit: number, offset: number) {
    ...
    return this.apiService.get<any[]>(this.TRAJECTORY_URL);
  }
   ...
   */
  getOne(id: number) {
    return this.apiService.get<any>(`${this.TRAJECTORY_URL}/${id}`);
  }
  ...
  getUserTrajectoryInfo() {
    return this.apiService.get<any>(`${this.TRAJECTORY_URL}/user-trajectory/`);
  }
   ...
   */
  getIndividualSkills() {
    return this.apiService.get<any>(`${this.TRAJECTORY_URL}/individual-skills/`).pipe(
      map(response => {
