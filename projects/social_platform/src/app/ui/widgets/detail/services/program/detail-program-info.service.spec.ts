...
describe("DetailProgramInfoService", () => {
  ...
  function setCurrentApplication(
    ...
    programUI.program.set(program({ id: 12, isUserMember: true, currentProjectApplication }));
  }

  it("currentProjectApplication=null offers application creation", () => {
  ...
  it("successful create sets a local draft and subsequent click opens it without a second POST", () => {
    ...
    expect(programUI.program()?.currentProjectApplication).toEqual(service.application());
  ...
  it("multi-program state trusts Program B currentProjectApplication instead of legacy project Program A", () => {
