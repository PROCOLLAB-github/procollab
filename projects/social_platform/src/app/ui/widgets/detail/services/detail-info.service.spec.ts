  ...
  it("uses the program application without waiting for the current profile", () => {
    ...
    const program = signal(
      Object.assign(Program.default(), {
        id: 12,
        isUserMember: true,
        currentProjectApplication: { projectId: 55, programLinkId: 700, submitted: false },
      }),
    );
    const application = computed(() => program().currentProjectApplication);
    ...
    program.update(current => ({
      ...current,
      currentProjectApplication: { ...current.currentProjectApplication!, submitted: true },
    }));
